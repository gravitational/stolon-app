...
sed -i "s/username = \"telegraf\"/username = \"${INFLUXDB_TELEGRAF_USERNAME}\"/" /etc/telegraf/telegraf.conf
# setup and start telegraf
echo "  address = \"host=stolon-postgres.default.svc user=${STOLON_USERNAME} password=$(cat $STOLON_PASSWORDFILE) sslmode=require dbname=postgres\"" \
